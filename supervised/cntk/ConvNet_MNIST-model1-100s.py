...
data_path  = os.path.join(abs_path,"MNIST_data") #"..", "..", "..", "DataSets", "MNIST")
...
def convnet_mnist(debug_output=False):
    ...
    with cntk.layers.default_options(activation=cntk.ops.relu, pad=False): 
        conv1 = cntk.layers.Convolution2D((3,3), 32, pad=True)(scaled_input)
        pool1 = cntk.layers.MaxPooling((3,3), (2,2))(conv1)
#        conv2 = cntk.layers.Convolution2D((3,3), 32)(pool1)
#        pool2 = cntk.layers.MaxPooling((3,3), (2,2))(conv2)
#	drop3 = cntk.layers.Dropout(0.25)(pool2)
#        conv3 = cntk.layers.Convolution2D((3,3), 64)(pool2)
#        f4    = cntk.layers.Dense(128)(drop3)
#        drop4 = cntk.layers.Dropout(0.5)(f4)
        z     = cntk.layers.Dense(num_output_classes, activation=None)(pool1)

#    ce = cntk.ops.cross_entropy_with_softmax(z, label_var)
    ce = CrossEntropyWithSoftmax(z, label_var)
    ...
    # training config
    epoch_size = 48000                    # for now we manually specify epoch size
    minibatch_size = 100
